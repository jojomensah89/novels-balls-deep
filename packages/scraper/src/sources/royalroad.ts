...
export class RoyalRoadSource extends BaseSource {
    ...
    private decoder = DECODERS.royalroad;
    ...
    async searchNovels(query: string): Promise<ScrapedNovel[]> {
        const searchUrl = `${this.baseUrl}/fictions/search?title=${encodeURIComponent(query)}`;
    ...
    async getNovel(url: string): Promise<ScrapedNovel> {
        ...
        if (statusText.includes("ongoing") || statusText.includes("active")) status = "ongoing";
